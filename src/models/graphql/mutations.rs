use crate::{
    models::graphql::{
        mutations::helpers::{insert_user, parse_cert, parse_signature, validate_challenge},
        types::{
            AuthenticatedUser, InvalidCertificate, InvalidChallenge, InvalidSignature,
            PrivateCertificate, PublicCertificate, SignupError, User,
        },
    },
    Config,
};
...
mod helpers {
    ...
    pub(super) async fn insert_user(
        ...
        let r = sqlx::query!(
            ...
            r#"
            WITH user_result AS (
                INSERT INTO users ( enc_cert, username, master_pw_hash ) 
                VALUES ( $1, $2, $3 )
                RETURNING id
            )
            INSERT INTO pubkeys ( user_id, fingerprint, pubkey ) VALUES (
                ( SELECT id from user_result ), $4, $5
            ) RETURNING (SELECT id FROM user_result)
            "#,
        ...
        match r {
            Ok(r) => {
                let id = ID(r.id.expect("The database should return an id").to_string());
                ...
                result.user = Some(User {
                    ...
                    certificate: PublicCertificate {
                        cert: cert.strip_secret_key_material(),
                    },
                    name: user.name,
                });
            }
                ...
                match e {
                    sqlx::Error::Database(e) => {
                        ...
                        match (e.code(), e.constraint()) {
                            ...
                            ("23505", Some("pubkeys_fingerprint_key")) => result.errors.push(
                            ...
                            ("23505", Some("pubkeys_pubkey_key")) => result.errors.push(
                                SignupError::CertificateTaken(CertificateTaken {
