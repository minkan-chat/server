use async_graphql::{Object, SimpleObject, InputObject, Union, ID};
...
impl PublicCertificate {
    pub(crate) async fn fingerprint(&self) -> String {
        "dummy fingerprint".to_string()
    }

    pub(crate) async fn content(&self) -> Bytes {
        Bytes(bytes::Bytes::from_static(&[0x0]))
    }
}
...
impl PrivateCertificate {
    pub(crate) async fn fingerprint(&self) -> String {
        "dummy fingerprint certificate".to_string()
    }
...
pub(crate) struct SignupUserInput {
    ...
    /// The certification of the server's PGP Certificate identity used to proof that the client is in control of the primary key of the Certificate
